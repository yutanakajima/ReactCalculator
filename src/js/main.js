...
const Button = (props) => (
  <button
    ...
    type="button"
    onClick={() => props.onButtonTouch(props.symbol)}>{props.symbol}</button>
);
...
class Panel extends React.Component {
  ...
  renderButtons(type, symbol) {
    ...
    type={type}
    symbol={symbol}
    onButtonTouch={this.props.onButtonTouch} /></td>;
  }
  renderZeroButton() {
    return <td colSpan="2"><Button
      type="default btn-zero"
      symbol={0}
      onButtonTouch={this.props.onButtonTouch} /></td>;
  }
  render() {
...
class Calculator extends React.Component {
  ...
  onButtonTouch(_button) {
    ...
    if (isNaN(_button)) {
      // 数値以外のボタンが押された場合 SAVE the World
      ...
      if (!this.state.symbol) {
        ...
        if (this.state.first_number === 0) {
          this.setState(() => {
            // 最初だけ押されたボタンの数値を入れる
            return {
              display_number: this.state.first_number = _button,
            };
          });
        } else {
          this.setState(() => {
            ...
            return {
              display_number: Number(
                this.state.first_number = '' +
                this.state.first_number + _button
              ),
            };
          });
        }
        ...
        if (this.state.second_number === 0) {
          this.setState(() => {
            return {
              display_number: this.state.second_number = _button,
            };
          });
        } else {
          this.setState(() => {
            return {
              display_number: Number(
                this.state.second_number = '' +
                this.state.second_number + _button
              ),
            };
          });
        }
